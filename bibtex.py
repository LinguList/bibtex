...
def lingpy_path(*comps):
    return os.path.join(os.path.dirname(lingpyd.__file__), *comps)
...
class BibTex(object):
    ...
    def __init__(self, bfile, jfile=''):
        ...
        if not bfile:
            bfile = lingpy_path('plugins', 'bibtex', 
                    'evobib','basic.bib')
        if not jfile:
            jfile = lingpy_path('plugins',
                    'bibtex', 'data', 'journals.csv')
    ...
    def _load_templates(self, path=''):
        ...
        if not path:
            path = lingpy_path('plugins', 'bibtex', 'data')
